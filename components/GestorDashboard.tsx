  ...
  return (
    <div className="p-4 md:p-8 space-y-10 animate-fade-in max-w-7xl mx-auto">
      <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-6">
        ...
        <div className="flex gap-4">
          <button 
            onClick={loadData}
            disabled={loading}
            className={`p-4 bg-white/5 rounded-2xl transition-all border border-white/5 text-xl ${loading ? 'animate-spin opacity-50' : 'hover:bg-white/10'}`}
            title="Sincronizar"
          >
            🔄
          </button>
      ...
      <section className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
        <MetricCard title="Saldo em Rede" value={loading ? '...' : metrics.totalCreditsInCirculation} unit="CR" icon="💰" color="text-blue-500" bg="bg-blue-600/5" />
        <MetricCard title="Suas Revendas" value={loading ? '...' : metrics.totalManaged} unit="Contas" icon="👥" color="text-purple-500" bg="bg-purple-600/5" />
        <MetricCard title="Clientes Finais" value={loading ? '...' : metrics.totalCustomers} unit="Ativos" icon="⚡" color="text-emerald-500" bg="bg-emerald-600/5" />
      </section>
        ...
        <div className="overflow-x-auto">
          <table className="w-full text-left">
            ...
            <tbody className="divide-y divide-gray-800/50">
              ...
              {loading && managedUsers.length === 0 && (
                <tr>
                  <td colSpan={3} className="px-8 py-10 text-center text-blue-500 uppercase font-black text-[10px] animate-pulse">Sincronizando dados...</td>
                </tr>
              )}
            </tbody>
          </table>
